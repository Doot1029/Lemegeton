const LemegetonPlugin = {
  ...
  commands: [
    ...
    {
      name: "ascii",
      help: "Switch to ASCII RPG mode.",
      fn: (args, engine) => {
        if (window.setGameMode) {
          window.setGameMode('ascii');
          engine.printLine("Switched to ASCII mode.");
        }
        return true;
      }
    },
    {
      name: "text",
      help: "Switch to Text Terminal mode.",
      fn: (args, engine) => {
        if (window.setGameMode) {
          window.setGameMode('text');
          engine.printLine("Switched to Text mode.");
        }
        return true;
      }
    }
...
module.exports = {
  ...
  rooms: [
    {
      id: 'mothers_cottage',
      name: '{{bold}}Mother\'s Cottage{{font_reset}}',
      description: 'A cozy, humble cottage where twins Deo and Dominique lived with their mother. The smell of fresh bread and lavender fills the air.',
      exits: {
        north: 'atrium'
      },
      asciiArt: `
