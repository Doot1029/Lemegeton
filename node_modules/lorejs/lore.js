  ...
  class Game {
    ...
    setupBrowserEvents() {
        this.inputElement.addEventListener('keydown', (e) => {
          if (e.key === 'Enter') {
            ...
            this.outputElement.scrollBy({ top: -50, behavior: 'auto' });
            e.preventDefault();
        ...
        // Global keydown listener for skipping animations
        window.addEventListener('keydown', (e) => {
          if (this.animationState.isAnimating && (e.key === 'Enter' || e.key === 'Escape')) {
            this.skipAnimation();
            e.preventDefault();
          }
        });
    ...
    processSingleTag(tag) {
      ...
      if (this.env === 'browser') {
        ...
        } else if (tag === 'final_fantasy_vii' || tag === 'ff') {
          result += '<span style="font-family: \'silver\', sans-serif">';
        } else if (tag === 'dungeon_mode' || tag === 'dm') {
          result += '<span style="font-family: \'dungeon mode\', sans-serif">';
        } else if (tag === 'infernal_script' || tag === 'is') {
          result += '<span style="font-family: \'infernal script\', sans-serif">';
        }
      } else {
        if (tag === 'font_reset' || tag === 'fr') {
          result += ANSI_STYLES.reset;
    ...
    processOutputQueue() {
      ...
      this.animationState.currentItem = nextItem;
    ...
    animateText(text, callback) {
      ...
      if (this.env === 'browser') {
        ...
        this.animationState.currentDiv = div;
        this.animationState.currentAnimation = setInterval(() => {
          this.animationState.currentIndex = index;
          if (index >= text.length) {
            clearInterval(this.animationState.currentAnimation);
            this.animationState.currentAnimation = null;
            this.animationState.currentItem = null;
            this.animationState.currentDiv = null;
    ...
    skipAnimation() {
      ...
      if (this.animationState.currentAnimation && this.animationState.isAnimating) {
        ...
        if (this.animationState.currentItem && !this.animationState.currentItem.img) {
          const item = this.animationState.currentItem;
          const text = typeof item.text === 'string' ? item.text : item.text[0];
          const formattedText = this.parseFormatting(text);
          if (this.env === 'browser' && this.animationState.currentDiv) {
            this.animationState.currentDiv.innerHTML = formattedText;
            this.outputElement.scrollTop = this.outputElement.scrollHeight;
        ...
        while (this.outputQueue.length > 0) {
          ...
          const text = typeof animationItem.text === "string" ? animationItem.text : animationItem.text[0];
          const formattedText = this.parseFormatting(text);
          if (this.env === 'browser') {
            const div = document.createElement('div');
            div.innerHTML = formattedText;
    ...
    look(silent = false) {
      if (this.state.flags.loadingSave) return;
      ...
      if (room.name && room.name.length) {
        this.printLine(`{{dm}}${room.name}{{fr}}`);
        this.printLine("");
      }
    ...
    async loadPlugin(plugin) {
      try {
        ...
        if (plugin.id && !this.plugins.has(plugin.id)) {
          ...
          return true;
        } else if (plugin.id && this.plugins.has(plugin.id)) {
          return true;
        } else {
    ...
    async prepareModule(content) {
      ...
      const isPath = content.startsWith('./') || content.startsWith('../') || content.includes('/') || content.includes('\\');
      if (Utils.isURL(content) || isPath) {
    ...
    registerDefaultCommands() {
      ...
      this.registerCommand({
        ...
        fn: (args, engine) => {
          ...
          const setTopicFlag = (topicData) => {
            if (topicData.setFlag) {
              engine.state.flags[topicData.setFlag] = true;
            }
          };
          const speakTopic = (topicKey, topicData, isRandom = false) => {
            ...
            if (isRandom) {
              const topicLabel = topicData.label || topicKey.replace(/_/g, " ");
              const intro = typeof character.randomTopicIntro === "function"
                ? character.randomTopicIntro(topicLabel, topicData, engine.state, engine)
                : `Hm, let's go with ${topicLabel}.`;
              if (intro) {
                engine.printLine(intro);
              }
            }
            topicData.dialog(engine.state, engine);
            setTopicFlag(topicData);
            return true;
          };
          // Handle topic-based conversation
          if (topic && character.topics) {
            if (topic === "random") {
              const availableTopics = Object.entries(character.topics).filter(([, topicData]) => !topicData.condition || topicData.condition(engine.state));
              if (availableTopics.length === 0) {
                engine.printLine(`${character.shortName || character.name} has nothing to say right now.`);
                return;
              }
              const [topicKey, topicData] = availableTopics[Math.floor(Math.random() * availableTopics.length)];
              speakTopic(topicKey, topicData, true);
              return;
            }
